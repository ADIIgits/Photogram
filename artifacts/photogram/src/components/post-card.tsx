...
import { getListPostsQueryKey, getGetFeedQueryKey, getGetDiscoverQueryKey, getGetUserPostsQueryKey, getGetPostQueryKey } from "@workspace/api-client-react";
...
export function PostCard({ post }: { post: Post }) {
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      queryClient.setQueryData(getGetFeedQueryKey(), updateCache);
      queryClient.setQueryData(getGetDiscoverQueryKey(), updateCache);
      queryClient.setQueryData(getGetUserPostsQueryKey(post.userId), updateCache);
      ...
      <div className="mt-4 px-2 md:px-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-4">
            ...
            >
              <Heart className={`w-6 h-6 transition-all ${isLiked ? "fill-destructive text-destructive" : "group-hover/btn:scale-110"}`} />
              <span className="text-sm font-medium">{post.likesCount}</span>
            </button>
            <Link href={`/post/${post.id}`} className="flex items-center gap-1.5 text-foreground/80 hover:text-foreground transition-colors group/btn">
