    ...
    <div className="min-h-[100dvh] bg-background text-foreground flex flex-col md:flex-row">
      ...
      <nav className="hidden md:flex flex-col w-64 border-r border-border p-6 h-screen sticky top-0 shrink-0">
        ...
        <div className="flex flex-col gap-6 flex-1">
          ...
          {user && (
            <NavLink href={`/profile/${user.id}`} icon={<User className="w-5 h-5" />} label="Profile" active={location === `/profile/${user.id}`} />
          )}
          ...
          <div className="mt-auto">
            {user ? (
              <div className="flex flex-col gap-4">
                ...
                <button onClick={handleLogout} className="flex items-center gap-4 text-muted-foreground hover:text-destructive transition-colors p-2 -mx-2 rounded-md hover:bg-muted/50 w-full text-left">
                  <LogOut className="w-5 h-5" />
                  <span className="font-medium tracking-wide">Log Out</span>
                </button>
              </div>
            ) : (
              <Link href="/login" className="block text-center bg-primary text-primary-foreground px-4 py-2 font-medium tracking-wide hover:bg-primary/90 transition-colors rounded-sm">
                Sign In
              </Link>
            )}
          </div>
      ...
      {/* Main Content */}
      <main className="flex-1 pb-20 md:pb-0 min-w-0 max-w-full">
        {children}
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t border-border bg-background/80 backdrop-blur-xl z-50 flex justify-around items-center h-16 px-4">
        ...
        {user ? (
          <>
            ...
            <MobileNavLink href={`/profile/${user.id}`} icon={<User className="w-6 h-6" />} active={location === `/profile/${user.id}`} />
          </>
        ) : (
...
function NavLink({ href, icon, label, active }: { href: string; icon: React.ReactNode; label: string; active: boolean }) {
  return (
    <Link href={href} className={`flex items-center gap-4 p-2 -mx-2 rounded-md transition-colors ${active ? "text-foreground font-medium bg-muted/20" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}>
      {icon}
